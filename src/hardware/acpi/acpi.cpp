...
Firework::FireworkKernel::Acpi::RsdpInfo bios_detect_rsdp(uint64_t base, size_t length) {
    uint64_t address = base + virtual_kernel_base;
    ...
    for (size_t off = 0; off < length; off += 16) {
        ...
        if (!rsdp->revision) {
            info.version = 1;
            info.address = (uint64_t)rsdp->rsdt_address + virtual_kernel_base;
            break;
        } else {
            ...
            info.version = 2;
            info.address = xsdp->xsdt_address + virtual_kernel_base;
            break;
        }
    }
...
void Firework::FireworkKernel::Acpi::init() {
    uint16_t* ebda_seg_ptr = (uint16_t*)(0x40E + virtual_kernel_base);

    Vmm::map_pages(Vmm::get_current_context(), 0x40E + virtual_kernel_base, 0x40E, (sizeof(uint16_t) + page_size - 1) / page_size, Vmm::VirtualMemoryFlags::VMM_PRESENT);
    ...
    if (rsdp_info.version >= 2) {
        ...
        for (size_t i = 0; i < entries; i++) {
            ...
            Vmm::map_pages(Vmm::get_current_context(), xsdt->tables[i] + virtual_kernel_base, xsdt->tables[i], 1, Vmm::VirtualMemoryFlags::VMM_PRESENT);

            SdtHeader* h = (SdtHeader*)(xsdt->tables[i] + virtual_kernel_base);

            Vmm::map_pages(Vmm::get_current_context(), xsdt->tables[i] + virtual_kernel_base, xsdt->tables[i], (h->length + page_size - 1) / page_size + 2, Vmm::VirtualMemoryFlags::VMM_PRESENT);

            if (calculate_checksum(h, h->length) == 0) {
                char text[255] = "";

                sprintf(text, "[ACPI] Found table with address %x and signature %c%c%c%c", (uint64_t)h - virtual_kernel_base, h->signature[0], h->signature[1], h->signature[2], h->signature[3]);
                Terminal::write_line(text, 0xFFFFFF);
        ...
        for (size_t i = 0; i < entries; i++) {
            ...
            Vmm::map_pages(Vmm::get_current_context(), (uint64_t)rsdt->tables[i] + virtual_kernel_base, (uint64_t)rsdt->tables[i], 1, Vmm::VirtualMemoryFlags::VMM_PRESENT);

            SdtHeader* h = (SdtHeader*)((uint64_t)rsdt->tables[i] + virtual_kernel_base);

            Vmm::map_pages(Vmm::get_current_context(), (uint64_t)rsdt->tables[i] + virtual_kernel_base, (uint64_t)rsdt->tables[i], (h->length + page_size - 1) / page_size + 2, Vmm::VirtualMemoryFlags::VMM_PRESENT);
